...
> 🗺️ **Don't want to type coordinates by hand? Use the online map picker:**
...
4. Select **"From URL"** and paste the following address:
   ```
   https://raw.githubusercontent.com/flavedo/ios-location-spoofer/main/ios-location-spoofer.sgmodule
   ```
...
### 1. Change location (One-Step Shortcut, Recommended ⚡)

The Worker now handles map short link expansion, coordinate conversion to WGS-84, terrain altitude lookup, and direct 302 redirection to the local proxy save URL automatically. You only need **1 action**:

1. Open the **Shortcuts app** → **+**
2. Tap the shortcut name → **Details** → enable **Show in Share Sheet**, accept **URLs** and **Text**
3. Add **Get Contents of URL** action, set URL to:
   ```
   YOUR_URL/api/set?u=[Shortcut Input]
   ```
4. Add **Show Notification** action → "✓ Location switched and applied"

> **How it works**: The Worker resolves the map link (Amap/Apple/Baidu/Google), fetches altitude, and returns an HTTP 302 redirect to `gs-loc.apple.com/ils-settings/save`. When Shortcuts follows the redirect, your local proxy tool intercepts the request and saves the location to your device in 1 second!

---

### 2. Multi-step Customization (if you want to inspect coordinates)

1. **Get Contents of URL**: `YOUR_URL/api/parse?format=json&u=[Shortcut Input]`
   Returns: `{ "lat": 24.49104, "lon": 118.080365, "alt": 46, "name": "...", "save_url": "..." }`
2. **Get Dictionary Value** → key `save_url`
3. **Get Contents of URL** → request `save_url`
4. **Show Notification** → "✓ Location applied"

**Usage**: Pick a place in Amap/Apple Maps/Google Maps → Share → choose this shortcut.
Or open `https://YOUR_URL/set?u=https://surl.amap.com/5tzQPeOZ9yA` in Safari to preview and apply in 1 tap!
